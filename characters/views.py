...
from .models import (
    Character,
)  # Import the Character model from the current app's models.py
...
def first_view(request):
    return HttpResponse(
        "Hello Ninjas!"
    )  # Returns a simple HTTP response saying "Hello Ninjas!"
...
def create_character(request):
    ...
    print(class_options)
    return render(
        request, "create_character.html", context={"class_options": class_options}
    )  # Renders the create_character.html template
    ...
    if request.method == "POST":  # Check if the request method is POST
        character_data = {
            "name": request.POST.get(
                "name"
            ),  # Retrieves the 'name' value from POST data
            "char_class": request.POST.get(
                "char_class"
            ),  # Retrieves the 'char_class' value from POST data
        ...
        request.session[
            "character_data"
        ] = character_data  # Store the character data in session
        new_char = Character.objects.create(
            **character_data
        )  # Create a new Character instance with the POST data
        return redirect("dashboard")  # Redirect to the 'dashboard' view
    return render(
        request, "dashboard"
    )  # If not a POST request, render the dashboard template


def view_character(request, char_id):
    character = Character.objects.get(
        id=char_id
    )  # Retrieve the Character instance with the given char_id
    context = {
        "character": character  # Context variable 'character' to pass to the template
    }
    return render(
        request, "view_character.html", context
    )  # Render the view_character.html template with the context
...
def edit_character(request, char_id):
    ...
    return render(
        request, "edit_character.html", context
    )  # Render the edit_character.html template with the context
...
def update_character(request, char_id):
    if request.method == "POST":  # Check if the request method is POST
        ...
        for key, value in character_data.items():
            setattr(
                character, key, value
            )  # Update each field of the character with new values
